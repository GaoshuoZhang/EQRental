...
CREATE TABLE Employee(
	empUsn VARCHAR(20) PRIMARY KEY,
	empPwd VARCHAR(40) NOT NULL
)

INSERT INTO Employee VALUES ('gz56','1234');
...
DROP VIEW IF EXISTS Rent_Rec
GO
CREATE VIEW Rent_Rec AS
SELECT rentEquipment.rRentalID,rentEquipment.rEquipmentID,Equipment.TypeName,Rental.startTime,rentEquipment.returnTime,Rental.hireFrom,rentEquipment.returnTo,Rental.CustomerEmail FROM rentEquipment
...
SELECT * FROM Rent_Rec

SELECT TypeName,BranchName,purchaseDate,EquipmentID FROM Equipment WHERE TypeName LIKE '%Dril%' ORDER BY purchaseDate DESC


SELECT fname,sname,phone FROM Customer WHERE Email = 'alex.johnson@example.com'

SELECT Equipment.EquipmentID,Equipment.TypeName,Equipment.BranchName,EquipmentType.ratePerHour FROM Equipment
JOIN EquipmentType ON Equipment.TypeName = EquipmentType.TypeName
WHERE Equipment.EquipmentID =3

INSERT INTO Customer (Email, fname, sname, phone) VALUES ('[txtBookEmail]', '[txtBookFname]', '[txtBookLname]', '[txtBookPhone]')

DELETE FROM rentEquipment WHERE rRentalID > 1000
DELETE FROM Rental WHERE RentalID > 1000
DELETE FROM Customer WHERE Email = 'gaoshuonz@gmail.com'
UPDATE Equipment SET available = 1 WHERE available = 0 
GO

CREATE PROCEDURE Return_Query
    @returnTime DATETIME = NULL,
    @rRentalID INT = NULL,
    @rEquipmentID INT = NULL,
    @Email NVARCHAR(255) = NULL,
    @phone NVARCHAR(50) = NULL
AS
BEGIN
    IF @returnTime IS NULL
    BEGIN
        SET @returnTime = GETDATE();
    END

    SELECT 
        rentEquipment.rRentalID,
        rentEquipment.rEquipmentID,
        Equipment.TypeName,
        Rental.hireFrom,
        Rental.startTime,
        EquipmentType.ratePerHour,
		CASE
            WHEN @returnTime > Rental.startTime
            THEN CEILING(DATEDIFF(HOUR, Rental.startTime, @returnTime))
            ELSE 0
        END AS RentHours,
        CASE
            WHEN CEILING(DATEDIFF(HOUR, Rental.startTime, @returnTime)) * EquipmentType.ratePerHour < 0
            THEN 0
            ELSE CEILING(DATEDIFF(HOUR, Rental.startTime, @returnTime)) * EquipmentType.ratePerHour
        END AS Cost,
    ...
    FROM 
        rentEquipment
    JOIN 
        Rental ON Rental.RentalID = rentEquipment.rRentalID
    JOIN 
        Equipment ON Equipment.EquipmentID = rentEquipment.rEquipmentID
    JOIN 
        EquipmentType ON EquipmentType.TypeName = Equipment.TypeName
    JOIN 
        Customer ON Customer.Email = Rental.CustomerEmail
    WHERE 
        rentEquipment.returnTime IS NULL 
        AND rentEquipment.returnTo IS NULL
        AND (@rRentalID IS NULL OR rentEquipment.rRentalID = @rRentalID)
        AND (@rEquipmentID IS NULL OR rentEquipment.rEquipmentID = @rEquipmentID)
        AND (@Email IS NULL OR Rental.CustomerEmail = @Email)
        AND (@phone IS NULL OR Customer.phone = @phone);
END
GO

EXEC Return_Query;
EXEC Return_Query @returnTime = '2024-06-01 12:00:00' 
GO

DROP PROCEDURE IF EXISTS Return_Query 
GO

CREATE TRIGGER trg_UpEquReturn
ON rentEquipment
AFTER UPDATE
AS
BEGIN
    ...
    SET 
        available = 1,
        BranchName = inserted.returnTo
    FROM 
        Equipment
    INNER JOIN 
        inserted ON Equipment.EquipmentID = inserted.rEquipmentID
    INNER JOIN 
        deleted ON deleted.rEquipmentID = inserted.rEquipmentID
    WHERE 
        inserted.returnTime IS NOT NULL 
        AND inserted.returnTo IS NOT NULL
        AND deleted.returnTime IS NULL 
        AND deleted.returnTo IS NULL;
END;
GO

CREATE VIEW UniqueCustomerCount_2023 AS
SELECT 
    R.hireFrom AS BranchName,
    COUNT(DISTINCT R.CustomerEmail) AS UniqueCustomerCount
FROM 
    Rental R
WHERE 
...
SELECT * FROM UniqueCustomerCount_2023
GO

CREATE VIEW AvgRentalDurationByEquType_2023 AS
SELECT 
    Equipment.TypeName,
    AVG(ABS(DATEDIFF(HOUR, Rental.startTime, ISNULL(rentEquipment.returnTime, GETDATE())))) AS AvgDuration
FROM 
    rentEquipment
JOIN 
...
WHERE 
    YEAR(Rental.startTime) = 2023 AND YEAR(rentEquipment.returnTime) = 2023
GROUP BY 
    Equipment.TypeName;
GO

SELECT * FROM AvgRentalDurationByEquType_2023
GO

CREATE VIEW MostPopularEqu_2023 AS
SELECT 
    BranchName,
    TypeName
FROM (
    SELECT
        r.hireFrom AS BranchName,
        e.TypeName,
        ROW_NUMBER() OVER (PARTITION BY r.hireFrom ORDER BY COUNT(*) DESC) AS rn
    FROM
        Rental r
    JOIN
...
WHERE rn = 1;
GO
SELECT BranchName, TypeName FROM MostPopularEqu_2023

DROP VIEW MonthlyRentalCounts_2023
GO
CREATE VIEW MonthlyRentalCounts_2023 AS
SELECT
    MONTH(startTime) AS Monthly,
...
CREATE VIEW TotalIncomeByEquType_2023 AS
SELECT 
    Equipment.TypeName,
    SUM(
        CASE 
            WHEN DATEPART(YEAR, Rental.startTime) = 2023
            THEN CEILING(ABS(DATEDIFF(HOUR, Rental.startTime, ISNULL(RE.returnTime, GETDATE())))) * EquipmentType.ratePerHour
            ELSE 0
        END
    ) AS TotalIncome
FROM 
    Equipment
JOIN 
    rentEquipment RE ON Equipment.EquipmentID = RE.rEquipmentID
JOIN 
    Rental ON RE.rRentalID = Rental.RentalID
JOIN 
    EquipmentType ON Equipment.TypeName = EquipmentType.TypeName
WHERE 
    DATEPART(YEAR, Rental.startTime) = 2023
GROUP BY 
    Equipment.TypeName;
GO

-- Tables
SELECT * FROM Equipment WHERE EquipmentID = 125;
SELECT * FROM rentEquipment WHERE rRentalID = 2046;
SELECT * FROM Rental;
SELECT * FROM EquipmentType;
SELECT * FROM Branch;
SELECT * FROM Customer;
...
----Summary
SELECT * FROM AvgRentalDurationByEquType_2023
SELECT * FROM MonthlyRentalCounts_2023
SELECT * FROM MostPopularEqu_2023
SELECT * FROM TotalIncomeByEquType_2023
SELECT * FROM UniqueCustomerCount_2023
GO
